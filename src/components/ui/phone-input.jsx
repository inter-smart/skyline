...
export const PhoneInput = ({
  ...
  const filteredCountries = countries.filter(
    (country) => country?.name?.toLowerCase().includes(searchTerm.toLowerCase()) || country?.mobileCode?.includes(searchTerm)
  );
  ...
  // Parse initial value or external value changes
  useEffect(() => {
  ...
  return (
    <div className="w-full relative m-auto" ref={dropdownRef}>
      <div className={cn("flex items-center h-full w-full relative", className)}>
        ...
        >
          ...
          />
          <span className="text-[11px] md:text-[12px] 2xl:text-[14px] 3xl:text-[16px] font-light text-white mr-1">{selectedCountry.mobileCode}</span>
          <svg viewBox="0 0 10 8" className={cn("w-[11px] h-[8px] transition-transform text-white", isOpen && "rotate-180")}>
            <path
      ...
      {isOpen && (
        <div
          className="absolute top-[calc(100%+7px)] left-0 mt-1 bg-white border border-gray-200 rounded-md shadow-lg 
              ...
              filteredCountries.map((country, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-[10px] 2xl:text-[12px] 3xl:text-[14px] text-gray-900 truncate mb-[3px]">{country.name}</div>
                    <div className="text-[10px] 2xl:text-[12px] 3xl:text-[14px] text-gray-500">{country.mobileCode}</div>
                  </div>
                </button>
              ))
            ) : (
              <div className="px-4 py-3 text-[10px] sm:text-[12px] xl:text-[14px] text-gray-500 text-center">No countries found</div>
            )}
          </div>
        </div>
      )}
